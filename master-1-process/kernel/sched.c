...
// 因为一个任务的数据结构与其内核态堆栈放在同一内存页中,所以从堆栈段寄存器ss可以获得其
// 数据段选择符.
union task_union {
...
// 前面的限定符volatile,英文解释是易改变的,不稳定的意思.
// 这个限定词的含义是向编译器指明变量的内容可能会由于被其他程序修改面变化.
// 通常在程序中声明一个变量时,编译器会尽量把它存放在通用寄存器中, 例如
// ebx,以提高访问效率.当CPU把其值放到ebx中后一般就不会再关心该变量对应内存位置中的内容.
// 若此时其他程序(例如内核程序或一个中断过程)修改了内存中
// 该变量的值,ebx中的值并不会随之更新.为了解决这种情况就创建了volatile限定符,
// 让代码在引用该变量时一定要从指定内存位置中取得其值.这里即是要求
// gcc不要对jiffies进行优化处理,也不要挪动位置,并且需要从内存中取其值.
// 因为时钟中断处理过程等程序会修改它的值.
...
int jiffies_offset = 0;								/* # clock ticks to add to get "true
													   ...
													   to WWV :-) */
/*
 * 为调整时钟而需要增加的时钟滴答,以获得"精确时间".这些调整用滴答数的总和不应该超过1秒.
 * 这样做是为了那些对时间精确度要求苛刻的人,他们培养喜欢
 * 自己的机器时间与WWV同步:-
 */
...
// 定义用户堆栈,共1K项,容量4K字节.在内核初始化操作过程中被用作内核栈,
// 初始化完成以后将被用作任务0的用户堆栈.在运行任务0之前它是内核栈,
// 以后用作任务0和1的用户态栈.
// 下面结构用于设置堆栈ss:esp(数据段选择符,指针).
// ss被设置为内核数据段选择符(0x10),指针esp指在user_stack数组最后一项后面.
// 这是因为Interl CPU执行堆栈操作时是先递减堆栈指针sp值,
// 然后在sp指针处保存入栈内容.
 ...
 */
// 当任务被调度交换过以后,该函数用以保存原任务的协处理器状态(上下文)并恢复新调度进来的当前任务的
// 协处理器执行状态.
 ...
 * 它不能被杀死,也不睡眠.任务0中的状态信息'state'是从来不用的.
 */
void schedule(void)
{
	int i, next, c;
...
// 函数参数p是等待任务队列头指针.指针是含有一个变量地址的变量.这里参数p使用了指针的指针形式'**p',
// 这是因为C函数参数只能传值,没有直接的方式让被调用函数改变调用该函数程序中变量的值.
// 但是指针'*p'指向的目标(这里是任务结构)会改变,因此为了能修改调用该函数程序中原来就是指针变量的值,
// 就需要传递指针'*p'的指针,即'**p'.
// 参数state是任务睡眠使用的状态:TASK_INTERRUPTIBLE或TASK_INTERRUPTIBLE.
// 处于不可中断睡眠状态(TASK_UNINTERRUPTIBLE)的任务需要内核程序利用
// wake_up()函数明确唤醒之.处于可中断睡眠状态(TASK_INTERRUPTIBLE)可以通过信号,
// 任务起时等手段唤醒(置为就绪状态TASK_RUNNING).
...
repeat:	schedule();
	...
	// 如果等待队列中还有等待任务,并且队列头指针*p所指向的任务不是当前任务时,说明
	// 在本任务插入等待队列后还有任务进入等待队列.于是我们应该也要唤醒这个任务,
	// 而我们自己应按顺序让这些后面进入队列的任务唤醒,因此这里将等待队列头所指任务先
	// 置为就绪状态,而自己则置为不可中断等待状态,即自己要等待这些后续队列的任务被唤醒而执行时来唤醒本任务.
	// 然后重新执行调度程序.
	if (*p && *p != current) {
		(**p).state = 0;
	...
	// 执行到这里,说明本任务真正被唤醒执行.此时等待队列头指针应该指向本任务,若它为空,则表明调度有问题,
	// 于是显示警告信息.最后我们让头指针指向在我们前面进入队列的
	// 任务(*p = tmp).若确实存在这样一个任务,即队列中还有任务(tmp不为空),就唤醒之.
	// 最先进入队列的任务在唤醒后运行时最终会把等待队列头指针置成NULL.
	if (!*p)
...
// 这种类型定时器类似现代Linux系统中的动态定时器(Dynamic Timer),
// 仅供内核使用.
