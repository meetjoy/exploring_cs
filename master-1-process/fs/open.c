...
// 打开(或创建)文件系统调用.
// 参数filename是文件名,flag是打开文件标志,它可取值:O_RDONLY(只读),O_WRONLY(只写)或O_RDWR(读写),
// 以及O_CREAT(创建),
// O_EXCL(被创建文件必须不存在),O_APPEND(在文件尾添加数据)等其他一些标志的组合,
// 如果本调用创建了一个新文件,则mode就用于指定文件的许可属性.
// 这些属性有S_IRWXU(文件宿主具有读,写和执行权限),S_IRUSR(用户具有读文件权限),
// S_IRWXG(组成员有读,写
// 执行)等等.对于新创建的文件,这些属性只应用于将来对文件的访问,
// 创建了只读文件的打开调用也将返回一个读写的文件句柄.如果调用
...
int sys_open(const char * filename, int flag, int mode)
{
	...
	// 首先对参数进行处理.将用户设置的文件模式和进程模式屏蔽码相与,产适配器的文件模式.
	// 为了为打开文件建立一个文件句柄,需要搜索进程
	// 结构中文件结构指针数组,以查找一个空闲项.空闲项的索引号fd即是句柄值.
	// 若已经没有空闲项,则返回出错码(参数无效).
	...
	// 现在我们初始化打开文件的文件结构.设置文件结构属性和标志,置句柄引用计数为1,
	// 并设置i节点字段为打开文件的i节点,初始化文件读写指针为0.最后返回文件句柄号.
...
int sys_close(unsigned int fd)
{
	...
	if (!(filp = current->filp[fd]))
		return -EINVAL;
	// 现在置该文件句柄的文件结构指针为NULL. 若在关闭文件之前,对应文件结构中的句柄引用计数已经为0,
	// 则说明内核出错,停机. 否则将对应文件结构的引用计数减1.此时如果它还不为0,则说明有其他进程正在使用该文件,
	// 于是返回0(成功).如果引用计数已等于0,
