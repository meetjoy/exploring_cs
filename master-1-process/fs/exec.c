...
// 该函数是系统中断调用(int 0x80)功能号__NR_execve调用的函数.函数的参数是进入系统调用处理过程后直到调用本
// 系统调用处理过程和调用本函数之前逐步压入栈中的值.这些值包括:
	...
	char ** argv, char ** envp)
{
	...
	*(filename1 + index + 1) = '\0';
	...
	// 在正式设置执行文件的运行环境之前,让我们先干些杂事.内核准备了128KB(32个页面)空间来存放正执行文件的
	// 命令行参数和环境字符串.
	// 上行把p初始设置成位于128KB空间的最后1个长字处.在初始参数和环境空间的操作过程中,p将用来指明在128KB空间
	// 中的当前位置.
	// 另外,参数eip[1]是调用本次系统调用的原用户程序代码段寄存器CS值,其中的段选择符当然必须是当前任务的
	// 代码段选择符(0x000f).
	// 若不是该值,那么CS只能会是内核代码段的选择符0x0008.但这是绝对不允许的,因为内核代码是常驻内存而不能
	// 被替换掉的.因此下面根据
...
restart_interp:
	...
	// 下面检查当前进程是否有权运行指定的执行文件.即根据执行文件i节点中的属性,看看本进程是否有权执行它.
	// 在把执行文件i节点的属性
	// 字段值取到i中后,我们首先查看属性中是否设置了"设置-用户-ID"(set-user-ID)标志和"设置-组-ID)
	// (set-group-id)标志.这两
	// 个标志主要是让一般用户能够执行特权用户(如超级用户root)的程序,例如改变密码的程序passwd等.
	// 如果set-user-ID标志置位,则
	// 后面执行进程的有效用户ID(euid)就设置成执行文件的用户ID,否则设置成当前进程的euid.
	// 如果执行文件set-group-id被置位的话,
	// 则执行进程的有效组ID(egid)就设置为执行执行文件的组ID.否则设置成当前进程的egid.
	//这里暂把这两个判断出来的值保存在变量
	// e_uid和e_gid中.
	i = inode->i_mode;
	...
	// 则把文件属性值i右移6位,此时其最低3
	// 位是文件宿主的访问权限标志.否则的话如果执行文件与当前进程的用户属于同组,
	// 则使属性最低3位是执行文件组用户的访问权限标志.否则
	// 此时属性字最低3位就是其他用户访问该执行文件的权限.然后我们根据属性字i的最低3位值来判断当前进程
	// 是否有权限运行这个执行文件.
	// 如果选出的相应用户没有运行该文件的权力(位0是执行权限),并且其他用户也没有任何权限或者当前进程用户
	// 不是超级用户,则表明当前进
	...
	// 程序执行到这里,说明当前进程有运行指定执行文件的权限.因此从这里开始我们需要取出执行文件头部数据并根据其
	// 中的信息来分析设置运行
	// 环境,或者运行另一个shell程序来执行脚本程序.首先读取执行文件第1块数据到高速缓冲块中.
	// 并复制缓冲块数据到ex中.如果执行文件开始
	// 的两个字节是字符"#!",则说明执行文件是一个脚本文件.如果想运行脚本文件,我们就需要执行脚本文件的解释程序
	// (例如shell程序).通常
	// 脚本文件的第一行文本为"#! /bin/bash".它指明了运行脚本文件需要的解释程序.运行方法是从脚本文件第1行
	// (带字符"#!")中取出其中
	// 的解释程序名及后面的参数(若有的话),然后将这些参数和脚本文件名放进执行文件(此时是解释程序)
	// 的命令行参数空间中.在这之前我们当然
	// 需要先把函数指定的原有命令行参数和环境字符串放到128KB空间中,而这里建立起来的命令行参数则放到
	// 它们前面位置处(因为是逆向放置).
	// 最后让内核执行脚本文件的解释程序.下面就是在设置好解释程序的脚本文件名等参数后,
	// 取出解释程序的i节点并跳转到229行支执行解释程序.
	// 由于我们需要跳转到执行过的代码229行支.因此在下面确认并处理了脚本文件之后需要设置一个禁止再次执
	// 行下面的脚本处理代码标志sh_bang.
	...
	if ((bh->b_data[0] == '#') && (bh->b_data[1] == '!') && (!sh_bang)) {
		...
		// 从这里开始，我们从脚本文件中提取解释程序名及其参数，并把解释程序名、解释程序的参数和脚本文件名
		// 组合放入环境参数块中。首先复制脚本
		// 文件头1行字符‘#!’后面的字符串到buf中，其中含有脚本解释程序名（例如/bin/sh），
		// 也可能还包含解释程序的几个参数。然后对buf中的内容
		// 进行处理。删除开始的空格、制表符。
		...
		// 此时我们得到了开头是脚本解释程序名的一行内容（字符串）。下面分析该行。首先取第一个字符串，
		// 它应该是解释程序名，此时i_name指向该
         ...
         */
		// 现在我们要把上面解析出来的解释程序名i_name及其参数i_arg和脚本文件名作为解释程序的参数放进环境和
		// 参数块中。不过首先
		// 我们需要把函数提供的原来一些参数和环境字符串先放进去，然后再放这里解析出来的。例如对于命令行参数来说，
		// 如果原来的参数
		// 是“-arg1 -arg2”、解释程序名是“bash”、其参数是“-iarg1 -iarg2”、脚本文件名（即原来的执行文件名）
		// 是“example.sh”
		// 那么在放入这里的参数之后，新的命令行类似于这样：
		//              “bash -iarg1 -iarg2 example.sh -arg1 -arg2”
		// 这里我们把sh_bang标志置上，然后把函数参数提供的原有参数和环境字符串放入到空间中。
		// 环境字符串和参数个数分别是envc和
		// argc-1个。少复制的一个原有参数是原来的执行文件名，即这里的脚本文件名。[[?? 可以看出，
		// 实际上我们不需要去另行处理脚本
		// 文件名，即这里完全可以复制argc个参数，包括原来执行文件名（即现在的脚本文件名）。
		// 因为它位于同一个位置上]]。注意！这里
		// 指针p随着复制信息增加而逐渐向小地址方向移动，因此这两个复制串函数执行完后，
		// 环境参数串信息块位于程序命令行参数串信息块
         ...
         */
		// 接着我们逆向复制脚本文件名、解释程序的参数和解释程序文件名到参数和环境空间中。若出错，则置出错吗，
		// 跳转到exec_error1。
		// 另外，由于本函数参数提供的脚本文件名filename在用户空间，但这里赋予copy_strings()的脚本文件名的
		// 指针在内核空间，因此
		// 这个复制字符串函数的最后一个参数（字符串来源标志）需要被设置成1。若字符串在内核空间，
		// 则copy_strings()的最后一个参数
		// 要设置成功，如下面。
	     ...
	     * OK，现在使用解释程序的i节点重启进程。
	     */
		// 最后我们取得解释程序的i节点指针，然后跳转到204行去执行解释程序。为了获得解释程序的i节点，
		// 我们需要使用namei()函数，但是该函数
		// 所使用的参数（文件名）是从用户数据空间得到的，即从段寄存器fs所指空间中取得。
		// 因此在调用namei()函数之前我们需要先临时让fs指向内
		// 核数据空间，以让函数能从内核空间得到解释程序名，并在namei()返回后恢复fs的默认设置。
		// 因此这里我们先临时保存原fs段寄存器（原指向
		// 用户数据段）的值，将其设置成指向内核数据段，然后取解释程序的i节点。之后再恢复fs的原值。
		// 并跳转到restart_interp（204行）处重新
		// 处理新的执行文件 -- 脚本文件的解释程序。
		old_fs = get_fs();
		set_fs(get_ds());
	...
	// 此时缓冲块中的执行文件头结构已经复制到了ex中.于是先释放该缓冲块,并开始对ex中的执行头信息进行判断处理.
	// 对于Linux0.12内核来说,
	// 它仅支持ZMAGIC执行格式,并且执行文件代码都从逻辑地址0开始执行,因此不支持含有代码或数据重定位信息的
	// 执行文件.当然,如果执行文件
	// 实在太大或者执行文件残缺不全,那么我们也不能运行它.因此对于下列情况将不执行程序:如果执行文件不是
	// 需求页可执行文件(ZMAGIC),或者
	// 代码和数据重定位部分长度不等于0,或者(代码段+数据段+堆)长度超过50MB,或者执行文件长度小于
	// (代码段+数据段+符号表长度+执行头部分)
	// 长度的总和.
	brelse(bh);
	...
	// 因为需求页(Demand paging)技术要求加载执行文件内容
	...
	// 若sh_bang标志已经设置,则表明将运行脚本解释程序,
	// 此时一环境变量页面已经复制,无须再复制.同样,若sh_bang没有置位而需要复制的话,
	// 那么此时指针p随着复制信息增加而逐渐向小地址方向移动,
	// 因此这两个复制串函数执行完后,环境参数串信息块位于程序参数串信息块的上方,并且p指向程序的第1个参数串.
	// 事实上,p是128KB参数和环境空间
	...
	// 但还没有为执行文件做过什么实质性的工作,即还没有做过为
	// 执行文件初始化进程任务结构信息,建立页表等工作.现在我们就来做这些工作.
	// 由于执行文件直接使用当前进程的"躯壳",即当前进程将被改造成执行
	// 文件的进程,因此我们需要首先释放当前进程占用的某些系统资源,
	// 包括关闭指定的已打开文件,占用的页表和内存页面等.然后根据执行文件头结构信
	// 息修改当前进程使用的局部描述符表LDT中描述符的内容,重新设置代码段和数据段描述符的限长,
	// 再利用当前处理得到的e_uid和e_gid等信息来设置
	// 进程任务结构中相关的字段.最后把执行本次系统调用程序的返回地址eip[]指向执行文件中代码的起始位置处.
	// 这样本系统调用退出返回后就会去运
	// 行新执行文件的代码了.注意,虽然此时新执行文件代码和数据还没有从文件中加载到内存中,
	// 但其参数和环境已经在copy_strings()中使用
	// get_free_page()分配了物理内存页来保存数据,并在chang_ldt()函数中使用put_page()到
	// 了进程逻辑空间的末端处.另外,在create_tables()
	// 中也会由于在用户栈上存放参数和环境指针表而引起缺页异常,从而内存管理程序也会就此为用户栈
	// 空间映射物理内存页.
	//
	// 这里我们首先放回进程原执行程序的i节点,并且让进程executable字段指向新执行文件的i节点.
	// 然后复位原进程的所有信号处理句柄,但对于SIG_IGN
	// 句柄无须复位.
	...
	// 然后根据当前进程指定的基地址和限长,释放原来程序的代码段和数据段所对应的内存页表指定的物理内存页面及页
	// 表本身.此时新执行文件并没有占用主
	// 内存区任何页面,因此在处理器真正运行新执行文件代码时就会引起缺页异常中断,
	// 此时内存管理程序即会执行缺页处理页为新执行文件申请内存页面和
	// 设置相关页表项,并且把相关执行文件页面读入内存中.如果"上次任务使用了协处理器"指向的是当前进程,
	// 则将其置空,并复位使用了协处理器的标志.
	...
	current->used_math = 0;
	// 然后我们根据新执行文件头结构中的代码长度字段a_text的值修改局部表中描述符基址和段限长,
	// 并将128KB的参数和环境空间页面放置在数据段末端.
	// 执行下面语句之后,p此时更改成以数据段起始处为原点的偏移值,但仍指向参数和环境空间数据开始处,
	// 即已转换成栈指针值.然后调用内部函数create_tables()
	// 在栈空间中创建环境和参数变量指针表,供程序的main()作为参数使用,并返回该栈指针.
	p += change_ldt(ex.a_text, page);
	p -= LIBRARY_SIZE + MAX_ARG_PAGES * PAGE_SIZE;
	p = (unsigned long) create_tables((char *)p, argc, envc);
	// 接着再修改进程各字段值为新执行文件的信息.即令进程任务结构代码尾字段end_code等于执行文件的代码段
	// 长度a_text;数据尾字段end_data等于
	// 执行文件的代码段长度加数据段长度(a_data + a_text);
	// 并令进程堆结尾字段brk = a_text + a_data + a_bss.brk用于指明进程当前数据段
	// (包括未初始化数据部分)末端位置,供内核为进程分配内存时指定分配开始位置.
	// 然后设置进程栈开始字段为栈指针所在页面,并重新设置进程的有效用户
	// id和有效组id.
