 ...
 */
/*
 * switch_to(n)将切换当前任务到任务nr,即n.首先检测任务n不是当前任务,如果是则什么也不做退出.
 * 如果我们切换的任务最近(上次运行)使用过数学协处理器的话,则还需复位控制器cr0中的TS标志.
 */
// 跳转到一个任务的TSS段选择符组成的地址处会造成CPU进行任务切换操作.
// 输入: %0 - 指向__tmp;		%1 - 指向__tmp.b处,用于存放新TSS 选择符.
//      dx - 新任务n的TSS选择符;  ecx - 新任务n的任务结构指针task[n].
// 其中临时数据结构__tmp用于组建远跳转(far jump)指令的操作数.该操作数由4字节偏移地址和2字节的段选择符组成.
// 因此__tmp中a的值是32位偏移值,而的低2字节是新TSS段的选择符(高2字节不用).
// 中转与TSS段选择符会造成任务切换到该TSS对应的进程.对于
// 造成任务切换的长跳转,a值无用.177行上的内存间接跳转指令使用6字节操作数作为跳转目的地的长指针,
// 其格式为:jmp 16位段选择符:32位偏移值.但在内存中操作数的表示顺序与这里正好相反.
// 任务切换回来之后,在判断原任务上次执行是否使用过协处理器时,
// 是通过将原任务指针与保存在last_task_used_math变更中的上次使用过协处理器指针进行比较而作出的,
// 参见文件kernel/sched.c
